...
2. 2K 和 4K 各测一组，并至少对 1K/8K 选项各做一次尺寸路径检查；不能动态降低分辨率或 DPR 来通过测试。内容修补输出必须跟随当前选择，不得固定为内部 2K。
...
| S6 | 按钮 1 画前表面蒙版，转到相反视角检查穿透；再绑定现成局部生图并连续应用、等待持久化 | 是 | 首笔/连续反馈、前后表面遮挡、发布长帧、测试前后图层与眼睛状态 |

S1–S6 不允许在操作期间停止自动旋转。手工复测时，一人持续拖动视口，另一输入设备执行图层操作；自动化测试使用 `perfOrbit=1` 提供确定性的持续相机运动。
...
5. 点击“清空”，等待 2 秒，收起面板，执行一个且仅一个 S1–S6 场景。
...
固定质量检查必须包含：滑槽内侧、壳体开孔、栏杆交界和底脚附近四类狭窄区域；第一轮与第二轮分别记录 requested/repaired/unresolved；下一轮为 0 缺口后仍需检查是否串入相邻表面的明显异色。冷证据轮和缓存命中轮分开计时，缓存 key 必须覆盖项目、对象、分辨率、图片/蒙版、相机、对象矩阵和全部图层调整。

### 局部重绘遮挡、眼睛与测试回滚

按钮 1 在模型背面画一条短蒙版，转到正面、内部和相反侧逐一检查；任何遮挡后条纹都判失败。关闭局部重绘图层眼睛后，普通 UV/投影层和实时 GPU 覆盖必须同帧消失；重新打开应恢复，不能要求删除图层。

S6 开始前导出图层 ID、顺序、visible 和 active layer，结束后逐项比较。成功、失败、超时均要求完全一致，且不存在新增的 `局部重绘 · 局部替换` 或空白测试层。遮挡深度分辨率、最终源图分辨率和顶部 1K/2K/4K/8K 选择同时记录，禁止通过降低深度/颜色分辨率通过性能门槛。
